#!/usr/bin/env bash
# Parallel MOSS v1.5 teacher WAV generation for major_2_03 (~10–30s clips).
set -euo pipefail
...
LOG_DIR="${LOG_DIR:-$ROOT/training/major_03}"
REF="${REF:-$ROOT/data/voices/major/major_2_03_cleaned.wav}"
CORPUS="${CORPUS:-$LOG_DIR/corpus/texts.jsonl}"
STAGGER_SEC="${STAGGER_SEC:-45}"
MIN_GPU_FREE_MB="${MIN_GPU_FREE_MB:-16000}"
WAV_STAGING="${WAV_STAGING:-/dev/shm/major03_wavs}"
...
# Longer clips: allow up to ~32s synthesis; QC window 9–32s.
...
export TEACHER_STYLES_MODULE=major_teacher_styles
...
if [[ ! -f "$CORPUS" ]]; then
  echo "Missing corpus: $CORPUS — run: python3 training/major_03/scripts/build_major03_corpus.py" >&2
  exit 1
fi
if [[ ! -f "$REF" ]]; then
...
moss_count() {
  pgrep -f '[m]oss-tts-server' 2>/dev/null | wc -l | tr -d ' '
}

stop_all() {
  pkill -f "build_realtime_finetune_dataset.py.*major_03" 2>/dev/null || true
  pkill -f "monitor_teacher_gen_resources.sh.*major_03" 2>/dev/null || true
...
echo "Stopping prior major03 teacher-gen workers..."
stop_all
...
mkdir -p "$LOG_DIR"
PIDS=()
WAV_REL="training/major_03/wavs"
for i in $(seq 0 $((NUM_SHARDS - 1))); do
  gpu="${GPU_ARR[$i]}"
  port="${PORT_ARR[$i]}"
  ...
  SPEAKER_ROOT="$ROOT" MOSS_RT_TRAIN_DIR="$LOG_DIR" PYTHONUNBUFFERED=1 \
    nohup python3 "${LEGACY}/build_realtime_finetune_dataset.py" \
      ...
      --out-dir "$LOG_DIR" \
      --teachers "$TEACHERS" \
      --teacher-styles major_teacher_styles \
      --wav-dir "$WAV_STAGING" \
