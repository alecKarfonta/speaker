...
All artifacts live under training/loli_15s/ (gitignored). Not part of the main repo PR.

Usage (from repo root):
  python training/loli_15s/scripts/distill.py env setup
  python training/loli_15s/scripts/distill.py corpus build
  python training/loli_15s/scripts/distill.py teacher gen --parallel
  python training/loli_15s/scripts/distill.py qc trim
...
def _prune_script(paths: Paths) -> Path:
    """Use moss-realtime prune (loli_15s/scripts/legacy copy can be stale)."""
    shared = paths.repo_root / "training/moss-realtime/scripts/legacy/prune_loli15s_teacher_dataset.py"
    if shared.is_file():
        return shared
    return paths.legacy_dir / "prune_loli15s_teacher_dataset.py"
...
def cmd_qc_report(paths: Paths, args: argparse.Namespace) -> int:
    ...
    extra = [
        ...
        "--wav-dir", str(wav_dir),
        "--corpus", str(corpus),
        "--qc-dir", str(paths.train_dir / "qc"),
        "--train-ids", str(paths.train_dir / "corpus/.no_train_ids"),
    ]
    return run_python(paths, _prune_script(paths), extra)
...
def _qc_prune_args(paths: Paths, args: argparse.Namespace, *, trim_only: bool) -> list[str]:
    ...
    extra = [
        ...
        "--qc-dir", str(paths.train_dir / "qc"),
        "--train-ids", str(paths.train_dir / "corpus/.no_train_ids"),
        "--workers", str(getattr(args, "workers", 4)),
    ]
    if trim_only:
...
def cmd_qc_trim(paths: Paths, args: argparse.Namespace) -> int:
    return run_python(paths, _prune_script(paths), _qc_prune_args(paths, args, trim_only=True))


def cmd_qc_prune(paths: Paths, args: argparse.Namespace) -> int:
    return run_python(paths, _prune_script(paths), _qc_prune_args(paths, args, trim_only=False))
...
def cmd_train_preprocess(paths: Paths, args: argparse.Namespace) -> int:
    ...
    prepared_out = os.environ.get("PREPARED_OUT")
    if prepared_out:
        extra["PREPARED_OUT"] = prepared_out
    if args.noref:
...
def build_parser() -> argparse.ArgumentParser:
    ...
    qt = qc_sub.add_parser("trim", help="STT tail-trim only (in-place on v15_pruned)")
    qt.add_argument("--limit", type=int, default=0)
    qt.add_argument("--workers", type=int, default=4)
    qt.add_argument(
        "--no-in-place",
        action="store_true",
