#!/usr/bin/env bash
# Parallel MOSS v1.5 teacher WAV generation for loli_15s batch2 (~3000 clips).
set -euo pipefail
...
LOG_DIR="${LOG_DIR:-$ROOT/training/loli_15s_batch2}"
...
# Never write into the original loli_15s teacher tree (st_* ~3k clips).
_legacy_loli="$ROOT/training/loli_15s"
if [[ "$LOG_DIR" == "$_legacy_loli" ]]; then
  echo "ERROR: LOG_DIR=$LOG_DIR would overwrite training/loli_15s — use training/loli_15s_batch2" >&2
  exit 1
fi
if [[ "$LOG_DIR" != *"loli_15s_batch2"* ]]; then
  echo "ERROR: LOG_DIR must be under training/loli_15s_batch2 (got $LOG_DIR)" >&2
  exit 1
fi
if [[ "$WAV_STAGING" == *"loli15s_wavs"* && "$WAV_STAGING" != *"batch2"* ]]; then
  echo "ERROR: WAV_STAGING=$WAV_STAGING is the legacy tmpfs path — use /dev/shm/loli15s_batch2_wavs" >&2
  exit 1
fi
...
stop_all() {
  # Only batch2 workers — do not pkill legacy loli_15s or major_03 processes.
  pkill -f '[b]uild_realtime_finetune_dataset.py.*loli_15s_batch2' 2>/dev/null || true
  pkill -f '[m]onitor_teacher_gen_resources.sh.*loli_15s_batch2' 2>/dev/null || true
...
echo "Stopping prior loli_15s_batch2 teacher-gen workers..."
stop_all
...
if [[ "$MONITOR" == "1" ]]; then
  pkill -f "monitor_teacher_gen_resources.sh.*loli_15s_batch2" 2>/dev/null || true
...
mkdir -p "$LOG_DIR"
PIDS=()
WAV_REL="training/loli_15s_batch2/wavs"
for i in $(seq 0 $((NUM_SHARDS - 1))); do
  gpu="${GPU_ARR[$i]}"
  port="${PORT_ARR[$i]}"
